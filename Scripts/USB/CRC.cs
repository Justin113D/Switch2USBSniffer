namespace Switch2USBSniffer.Data.USB
{
	...
	{
		private static readonly byte[] _crc5Table = [
			0x1e, 0x15, 0x03, 0x06, 0x0c, 0x18, 0x19, 0x1b,
			0x1f, 0x17, 0x07, 0x0e, 0x1c, 0x11, 0x0b, 0x16,
			0x05, 0x0a, 0x14
		];

		public static byte CRC5On11Bit(ushort v)
		{
			const int vl = 11;
			byte rv = 0x02;
		...
		{
			uint rv = ushort.MaxValue;

			for(int i = 0; i < data.Length; i++)
			{
				rv = _crc16Table[(rv ^ data[i]) & 0xff] ^ (rv >> 8);
			}

			return (ushort)(rv ^ ushort.MaxValue);
		}
	}
}
